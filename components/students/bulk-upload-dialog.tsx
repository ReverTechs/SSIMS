...
export function BulkUploadDialog({ open, onOpenChange, onSuccess }: BulkUploadDialogProps) {
    ...
    useEffect(() => {
        if (!open) {
            ...
            // Fetch class mapping when dialog opens
            getClassMapping().then(setClassMapping);
        }
    }, [open]);
    ...
    const handleUpload = async () => {
        if (!parsedData || !classMapping) return;

        setStep('processing');
        try {
            const result = await bulkRegisterStudents(parsedData.data, classMapping);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="w-[95vw] max-w-[95vw] sm:max-w-[95vw] h-[95vh] p-0 flex flex-col gap-0">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b">
                    <div>
                        <h2 className="text-lg font-semibold">Bulk Student Registration</h2>
                        <p className="text-sm text-muted-foreground">
                            {step === 'upload' && 'Upload your student data file'}
                            {step === 'preview' && 'Review data before registration'}
                            {step === 'processing' && 'Registering students...'}
                        ...
                        {step === 'preview' && (
                            <>
                                ...
                                <Button onClick={handleUpload} disabled={!parsedData?.validRowCount}>
                                    Upload {parsedData?.validRowCount} Students
                                </Button>
                            </>
                        )}
                        {step === 'result' && (
                        ...
                        <div className="flex flex-col items-center justify-center h-full space-y-4">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-muted-foreground">
                                    Please wait while we register the students.
