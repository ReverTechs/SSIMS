...
import type { BulkTeacherData, ValidationError } from '@/types/bulk-upload-types';

interface BulkUploadPreviewTableProps {
    data: BulkTeacherData[];
    errors: ValidationError[];
}

export function BulkTeacherUploadPreviewTable({ data, errors }: BulkUploadPreviewTableProps) {
    const [sorting, setSorting] = useState<SortingState>([]);
    const [globalFilter, setGlobalFilter] = useState('');

    // Define columns with compact styling
    const columns = useMemo<ColumnDef<BulkTeacherData>[]>(
        () => [
            {
                accessorKey: 'title',
                header: 'Title',
                cell: ({ row }) => <div className="text-sm px-2">{row.getValue('title')}</div>,
            },
            {
            ...
            },
            {
                ...
                header: 'Employee ID',
                cell: ({ row }) => (
                    <div className="font-mono text-sm px-2">{row.getValue('employeeId')}</div>
                ),
            },
            {
                ...
                header: 'Department',
                cell: ({ row }) => (
                    <div className="text-sm px-2">{row.getValue('department')}</div>
                ),
            },
            {
                accessorKey: 'role',
            ...
            },
            {
                ...
                header: 'Type',
                cell: ({ row }) => {
                    const type = row.getValue('teacherType') as string;
                    return (
                        <div className="px-2">
                            <Badge variant={type === 'permanent' ? 'default' : 'secondary'} className="capitalize">
                                {type}
                            </Badge>
                        </div>
                    );
            ...
            },
            {
                ...
                header: 'Subjects',
                cell: ({ row }) => (
                    <div className="text-sm truncate max-w-[150px] px-2" title={row.getValue('subjects')}>
            ...
            },
            {
                ...
                header: 'Classes',
                cell: ({ row }) => (
                    <div className="text-sm truncate max-w-[150px] px-2" title={row.getValue('classes')}>
